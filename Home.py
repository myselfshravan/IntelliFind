...
if option == 'Home':
    ...
    if submit_button:
            try:
                ...
                response = requests.post("https://in35zed2ta.execute-api.us-east-1.amazonaws.com/api/admin/found", files=payload)
    ...
    if submit_button1:
        ...
        # Send the GET request to retrieve image data
            response = requests.get("https://in35zed2ta.execute-api.us-east-1.amazonaws.com/api/client/getimages/843eab46-d153-4710-999f-06e5048cd1eb")
            st.write(response)
        # Check the response status code
            if response.status_code == 200:
